...
*   **Next.js / TypeScript**:
    ...
    *   **组件架构**: 优先使用服务端组件 (Server Components)，仅在需要用户交互（事件处理、状态管理、浏览器 API）时才使用客户端组件 (Client Components)。
    *   **TypeScript 严格模式**: 禁止使用 `any` 类型，所有代码必须定义明确的类型。对于未知类型使用 `unknown`，对于动态对象使用具体的接口或类型定义。
