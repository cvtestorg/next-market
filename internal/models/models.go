...
type PluginVersion struct {
	...
	ConfigSchemaJSON []byte         `gorm:"type:jsonb" json:"-"` // 不直接序列化
	ConfigSchema     interface{}    `gorm:"-" json:"config_schema_json"` // 用于 JSON 响应
	ConfigValuesJSON []byte         `gorm:"type:jsonb" json:"-"` // 存储配置值
	ConfigValues     interface{}    `gorm:"-" json:"config_values_json"` // 用于 JSON 响应
...
// ParseConfigSchema 解析 ConfigSchemaJSON 到 ConfigSchema
func (pv *PluginVersion) ParseConfigSchema() {
	if len(pv.ConfigSchemaJSON) > 0 {
		var schema interface{}
		if err := json.Unmarshal(pv.ConfigSchemaJSON, &schema); err == nil {
			pv.ConfigSchema = schema
		}
	}
}

// ParseConfigValues 解析 ConfigValuesJSON 到 ConfigValues
func (pv *PluginVersion) ParseConfigValues() {
	if len(pv.ConfigValuesJSON) > 0 {
		var values interface{}
		if err := json.Unmarshal(pv.ConfigValuesJSON, &values); err == nil {
			pv.ConfigValues = values
		}
	}
}
